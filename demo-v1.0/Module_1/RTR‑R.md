...
```bash
# Настойка hostname
hostnamectl set-hostname rtr-r.au-team.irpo; exec bash
```

1. Настройка сети
```bash
# В сторону клиенsrv-lболее 16 адресов /28
nmtui
```
...
- задаём «Локальный IP» (IP на интерфейсе BR-RTR в сторону IPS 172.16.222.2)
- задаём «Удалённый IP» (IP на интерфейсе HQ-RTR в сторону ISP 172.16.221.2)
...
2. Настройка динамической маршрутизации средствами FRR
```bash
dnf install -y frr
